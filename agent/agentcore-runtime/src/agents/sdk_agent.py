#!/usr/bin/env python3
"""
SDK Agent for Broker AgentCore Runtime with BedrockAgentCoreApp
Handles lending policy queries and broker operations
"""
...
logger.info(f"🚀 Broker Agent SDK (CLI deployable) starting with model: {model_settings['model_id']}")
if gateway_url:
...
def create_strands_agent(use_mcp=True):
    """Create Strands agent with local tools and optionally MCP tools for broker operations"""
    # Create BedrockModel
    model = BedrockModel(**model_settings)

    # Define system prompt for the broker agent
    system_prompt = """You are a Broker Agent specializing in lending policy analysis and loan application support.

Your primary capabilities include:
1. Retrieving and analyzing lending policies from knowledge bases
2. Providing guidance on loan application requirements
3. Assessing compliance with lending guidelines
4. Offering insights on loan structuring and risk factors

CRITICAL: For any date-related queries, ALWAYS use the get_current_time tool first to get the current date before making any calculations.

When using broker tools:
1. If the query involves dates or time periods:
   - FIRST call get_current_time to get the current date
   - Calculate accurate date ranges based on current time
   - Use specific dates in your analysis

2. For policy queries:
   - Use the retrieve_policy tool to search lending policies
   - Be specific and clear in your search queries
   - Focus on the most relevant policy information

3. Provide clear, structured responses that include:
...
- retrieve_policy: Search lending policies and guidelines

Focus on providing accurate, helpful guidance for mortgage brokers and lending professionals.
"""
    ...
    if use_mcp and gateway_url and is_mcp_available(gateway_url):
        try:
            ...
            if token_to_use:
                ...
                logger.info(f"🔗 MCP client ready - Broker Agent will have {len(tools)} total tools")
            else:
                logger.warning("⚠️ No OAuth access_token available for MCP")
                logger.info(f"🛠️ Broker Agent created with {len(tools)} local tools only")
        except Exception as e:
...
async def invoke(payload):
    ...
    logger.info("📥 Received Broker Agent SDK invocation request")
    ...
    try:
        ...
        if strands_agent is None:
            logger.warning("⚠️ Broker Agent not initialized, creating now...")
        ...
        logger.info(f"🚀 Broker Agent SDK invocation: {user_message[:50]}...")
        ...
        logger.info(f"🤖 Starting Broker Agent SDK stream...")
        ...
        logger.info(f"✅ Broker Agent SDK stream completed with {chunk_count} chunks, {tool_calls_detected} tool calls detected")
        ...
        logger.error(f"❌ Broker Agent SDK streaming error: {e}")
...
def initialize_services():
    ...
    logger.info("🚀 Starting Broker Agent SDK...")
    ...
    logger.info("✅ Broker Agent SDK ready")

def cleanup_resources():
    """Clean up resources on shutdown"""
    logger.info("🛑 Shutting down Broker Agent SDK...")
    ...
    logger.info("✅ Broker Agent SDK shutdown complete")
