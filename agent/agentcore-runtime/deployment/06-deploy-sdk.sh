...
# Deploy the Broker Agent SDK implementation to ECR
echo "🚀 Deploying Broker Agent SDK (BedrockAgentCoreApp)..."
...
ECR_REPO="broker-agent-runtime-repo-sdk"
...
if python3 deploy-sdk-runtime.py; then
    echo ""
    echo "🎉 Broker Agent SDK Deployment Complete!"
    echo "========================================"
    ...
    echo "   • OAuth integration with broker-agent-identity-provider-okta"
    echo "   • MCP client for broker-agent-gtw gateway"
    echo "   • AgentCore Memory integration"
    echo ""
    echo "💻 Your Broker Agent SDK is now ready to use OAuth2 tokens and MCP gateway!"
    echo "   Uses BedrockAgentCoreApp framework with @entrypoint decorator"
    echo "   Connect to the MCP gateway for Bedrock Knowledge Base tool access"
else
    echo ""
